...
class GatewayEndpointPermissionRegexRepository:
    ...
    def _get_endpoint_regex_permission(self, session: Session, id: int, user_id: int) -> SqlGatewayEndpointRegexPermission:
        try:
            return (
                session.query(SqlGatewayEndpointRegexPermission)
                .filter(
                    SqlGatewayEndpointRegexPermission.id == id,
    ...
    def grant(self, regex: str, priority: int, permission: str, username: str) -> GatewayEndpointRegexPermission:
        ...
        with self._Session() as session:
            try:
                ...
                perm = SqlGatewayEndpointRegexPermission(
                    regex=regex,
                    priority=priority,
                    user_id=user.id,
                ...
                raise MlflowException(
                    f"Gateway endpoint regex perm exists ({regex},{username}): {e}",
    ...
    def list_regex_for_user(self, username: str) -> List[GatewayEndpointRegexPermission]:
        with self._Session() as session:
            ...
            perms = (
                session.query(SqlGatewayEndpointRegexPermission)
                .filter(
                    SqlGatewayEndpointRegexPermission.user_id == user.id,
                )
                .order_by(SqlGatewayEndpointRegexPermission.priority)
                .all()
            )
